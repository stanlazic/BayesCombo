...
```{r BSfactor}
bs <- BSfactor(pmp, threshold = 0.95 )
summary(bs)
```

The plot below shows how the posterior model probabilities for each hypothesis change as the prior for the null increases.


```{r plotBS, fig.height=4,fig.width=6}
par(las=1)
plot(bs)
```
